...
class TestPaymentTransaction(SipagoCommon, PaymentHttpCommon):
    ...
    def test_no_item_missing_from_preference_request_payload(self):
        ...
        failed_url = urls.url_join(
            base_url, f'{SipagoController._return_url}?ref={sanitized_reference}&status=DENIED')
    ...
    def test_no_input_missing_from_redirect_form(self):
        ...
        tx = self._create_transaction(flow='redirect')
        with patch(
            'odoo.addons.payment_sipago.models.payment_transaction.PaymentTransaction'
    ...
    def test_processing_notification_data_confirms_transaction(self):
        ...
        with patch(
            ...
            '._sipago_make_request', return_value=self.verification_data_success
        ):
    ...
    def test_processing_notification_data_sets_pending(self):
        ...
        with patch(
            'odoo.addons.payment_sipago.models.payment_provider.Paymentprovider'
            '._sipago_make_request', return_value=self.verification_data_pending
        ):
            tx._process_notification_data(self.redirect_notification_data)
    ...
    def test_processing_notification_data_cancels_transaction(self):
        ...
        with patch(
            'odoo.addons.payment_sipago.models.payment_provider.Paymentprovider'
            '._sipago_make_request', return_value=self.verification_data_expired
        ):
            tx._process_notification_data(self.redirect_notification_data)
    ...
    def test_processing_notification_data_rejects_transaction(self):
        ...
        with patch(
            'odoo.addons.payment_sipago.models.payment_provider.Paymentprovider'
            '._sipago_make_request', return_value=self.verification_data_failed
        ):
            tx._process_notification_data(self.redirect_notification_data)
